...
def view_table():
    ...
    top.protocol("WM_DELETE_WINDOW", top_closes)
    top.title("Name of Columns")
    top.focus_set()
    col_window = Frame(top)
    col_window.grid(column=0, row=0, sticky=(N, W, E, S))
    ...
    if (numcol > 10):
        button_insert = Button(col_window, text="INSERT", command=lambda: insert_data(value))
        button_insert.grid(row=15, column=3, )

        button_update = Button(col_window, text="UPDATE", command=lambda: update_data(value))
        button_update.grid(row=15, column=4, )

        button_delete = Button(col_window, text="Delete", command=lambda: delete_data(value))
        button_delete.grid(row=15, column=5)

        button_search = Button(col_window, text="Fetch", command=lambda: search_data(value))
...
def print_data(value):
    ...
    h1 = document.add_heading(
        "District: " + district + "   Tehsil:  " + tehsil + "  Village:  " + village + "   Paragana:  " + paragana, 0)
    h1.alignment = 1
    ...
    table = document.add_table(rows=len(data), cols=len(list(data[0].keys())), style='LightShading-Accent1')
...
db_name = FirstWindow(connection).db_name
root = Tk()
